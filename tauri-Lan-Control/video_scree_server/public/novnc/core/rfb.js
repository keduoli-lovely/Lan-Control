...
export default class RFB extends EventTargetMixin {
  ...
  _writeClipboard(text) {
    ...
    if (
      window.__vncHook &&
      typeof window.__vncHook.onSendClipboard === "function"
    ) {
      window.__vncHook.onSendClipboard(text);
    }
    this.dispatchEvent(
  ...
  _handleServerCutText() {
    ...
    if (length >= 0) {
      ...
      if (
        window.__vncHook &&
        typeof window.__vncHook.onReceiveClipboard === "function"
      ) {
        window.__vncHook.onReceiveClipboard(text);
      }
      if (this._viewOnly) {
        return true;
      }
      ...
      if (isCaps) {
        ...
        if (textData !== null) {
          ...
          if (
            ...
            window.__vncHook.onReceiveClipboard(textData);
          }
...
RFB.messages = {
  keyEvent(sock, keysym, down) {
    if (window.__vncHook && typeof window.__vncHook.onKey === "function") {
      window.__vncHook.onKey(down, keysym);
    }
  ...
  pointerEvent(sock, x, y, mask) {
    if (window.__vncHook && typeof window.__vncHook.onPointer === "function") {
      window.__vncHook.onPointer(x, y, mask);
    }
